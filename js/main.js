...
function displayLesson() {
    ...
    exercisesContainer.innerHTML = lesson.exercises.map((exercise, index) => {
        const exerciseId = `exercise-${index}`;
        // Салыстыру есептері үшін text input, басқалары үшін number
        const isComparison = exercise.includes('?') && (exercise.includes('>') || exercise.includes('<') || exercise.includes('='));
        const isFraction = exercise.includes('/');
        const inputType = (isComparison || isFraction) ? 'text' : 'number';
        const placeholder = isComparison ? '>, <, немесе = енгізіңіз' : (isFraction ? 'Мысалы: 3/4' : 'Жауабыңызды енгізіңіз');
        // Экранирование для HTML и JavaScript
        const escapedExercise = exercise.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
        const jsEscapedExercise = exercise.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, '\\"');
        return `
            <div class="exercise-item">
                <p>${exercise}</p>
                <input type="${inputType}" id="${exerciseId}" class="exercise-input" placeholder="${placeholder}">
                <button onclick="checkExercise(${JSON.stringify(exercise)}, '${exerciseId}')" class="check-btn">Тексеру</button>
                <div id="${exerciseId}-result"></div>
            </div>
        `;
    }).join('');
...
function checkExercise(exercise, inputId) {
    ...
    if (exercise.includes('?') && (exercise.includes('>') || exercise.includes('<') || exercise.includes('='))) {
        if (!userAnswerRaw || (userAnswerRaw !== '>' && userAnswerRaw !== '<' && userAnswerRaw !== '=')) {
            resultDiv.innerHTML = '<div class="exercise-result incorrect">>, <, немесе = енгізіңіз!</div>';
            return;
        }
    }
    // Бөлшектер үшін
    ...
    // Салыстыру үшін (>, <, =)
    if (typeof correctAnswer === 'string' && (correctAnswer === '>' || correctAnswer === '<' || correctAnswer === '=')) {
    ...
    if (!isNaN(userAnswer) && !isNaN(correctAnswerNum) && Math.abs(userAnswer - correctAnswerNum) < 0.01) {
        resultDiv.innerHTML = '<div class="exercise-result correct">✅ Дұрыс! Жарайсың!</div>';
        input.style.borderColor = '#28a745';
    } else {
        resultDiv.innerHTML = `<div class="exercise-result incorrect">❌ Дұрыс емес. Дұрыс жауап: ${correctAnswer}</div>`;
